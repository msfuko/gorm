...
func (cp *CallbackProcessor) Register(callbackName string, callback func(scope *Scope)) {
	if cp.kind == "row_query" {
		if cp.before == "" && cp.after == "" && callbackName != "gorm:row_query" {
			defaultLogger.Print("log", "callbackProcessor.Register", fmt.Sprintf("Registering RowQuery callback %v without specify order with Before(), After(), applying Before('gorm:row_query') by default for compatibility...\n", callbackName))
...
func (cp *CallbackProcessor) Remove(callbackName string) {
	defaultLogger.Print("log", "callbackProcessor.Remove", fmt.Sprintf("removing callback `%v` from %v\n", callbackName, fileWithLineNum()))
	cp.name = callbackName
	cp.remove = true
	cp.parent.processors = append(cp.parent.processors, cp)
	cp.parent.reorder()
...
func (cp *CallbackProcessor) Replace(callbackName string, callback func(scope *Scope)) {
	defaultLogger.Print("log", "callbackProcessor.Replace", fmt.Sprintf("replacing callback `%v` from %v\n", callbackName, fileWithLineNum()))
	cp.name = callbackName
	cp.processor = &callback
	cp.replace = true
	cp.parent.processors = append(cp.parent.processors, cp)
...
func sortProcessors(cps []*CallbackProcessor) []*func(scope *Scope) {
	...
	for _, cp := range cps {
		...
		if index := getRIndex(allNames, cp.name); index > -1 && !cp.replace && !cp.remove {
			defaultLogger.Print("log", "sortProcessors", fmt.Sprintf("duplicated callback `%v` from %v\n", cp.name, fileWithLineNum()))
		}
